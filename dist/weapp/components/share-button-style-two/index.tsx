...
import './index.scss'
import {defaultPosterConfig} from'./canvasConfig.js'

interface Props {
  activityUrl: string // 活动图片链接
  activityDescription: string // 活动描述
  activityPrice: string // 活动价格
  activityOriginalPrice?:string // 活动原价
  pageUrlCode:string //页面二维码链接
  userDescription?:string // 用户头像旁边的描述
  posterGenerateSuccess?: (detail: any) => any // 海报生成成功后调用
  posterGenerateFail?: (e: any) => any // 海报生成失败后调用
  onCloseShareButton?:(message:any)=>any // 关闭模态框后调用
}

interface State {
  showModel: boolean // 控制显示模态框
  posterUrl: string //生成的海报图片链接
  generatePosterStatus:boolean // 生成海报是否成功
  config: any;
  canvasStatus: any; // 画布的状态
}
...
class ShareButton extends Component<Props, State> {
  ...
  render() {
    ...
    return (
      <View className='poster' onTouchMove={this.onMovie}>
        <Button
          className='children'
          openType='getUserInfo'
          onGetUserInfo={this.onShare.bind(this)}
        >
          {this.props.children}
        </Button>
        {showModel && (
          <View className='content-share'>
            ...
            <View className='poster'>
              {this.state.generatePosterStatus && (
                <Image
