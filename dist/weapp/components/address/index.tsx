...
class Address extends Component<Props, StateProps> {
  static options = {
    addGlobalClass: true
  };
  ...
  getProvinceAndCityObj(
    ...
    const city =
      province.code > 0
        ? REGION_DATA.city[province.code][cityIndex]
        : defaultObj;
    const area =
      city.code > 0 && REGION_DATA.area[city.code]
        ? REGION_DATA.area[city.code][areaIndex]
        : defaultObj;
    return {
      province,
      city,
  ...
  get regionValues() {
    ...
    const provinceIndex = REGION_DATA.province.findIndex(
      item => {
        const {code} = item;
    ...
    if (cityIndex > -1) {
      ...
      if (REGION_DATA.area[cityItem.code]) {
        areaIndex = REGION_DATA.area[cityItem.code].findIndex(
          item => item.code === area
        );
      }
    }
