...
export function downImage(imageUrl) {
  return new Promise((resolve, reject) => {
    if (/^http/.test(imageUrl) && !new RegExp(wx.env.USER_DATA_PATH).test(imageUrl) && !/^http:\/\/tmp/.test(imageUrl)) {
      Taro.downloadFile({
        url: imageUrl,
        success: res => {
...
export function downloadImageAndInfo(image, index, toRpxFunc, pixelRatio) {
  return new Promise((resolve, reject) => {
    ...
    const imageUrl = url;
    downImage(imageUrl).then(imgPath => getImageInfo(imgPath, index)).then(({ imgPath, imgInfo }) => {
      let sx;
      let sy;
