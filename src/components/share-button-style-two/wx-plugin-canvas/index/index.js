...
const helper = {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._downImage(imageUrl, index)
        ...
        .then(({ imgPath, imgInfo }) => {
          ...
          const width = imgInfo ? this.toRpx(imgInfo.width / pixelRatio) : image.width
          const height = imgInfo ? this.toRpx(imgInfo.height / pixelRatio) :image.height
          ...
          this.drawArr.push({
            ...
            imgPath:imgPath ? imgPath : imageUrl,
            sx,
            sy,
            sw: width - sx * 2,
            sh: height - sy * 2,
  ...
  _getImageInfo(imgPath, index) {
    return new Promise((resolve, reject) => {
      ...
      const reg = /qmfxSubpkg/
      if(reg.test(imgPath)){   //不是https的不用下载
         resolve(imgPath,void 0)
      }
