...
class CashierSuccess extends Taro.Component {
  render() {
    const { onPaySuccess, couponData, payFee } = this.props;

    return <View className="cashier-success-wrapper">
      ...
      <View className="cashier-success-main">
        ...
        {couponData && couponData.length > 0 && <View className="cashier-success-main-coupon">
            ...
            <View className="cashier-success-coupon-wrapper" style={{ width: '100%' }}>
              {couponData && couponData.length > 0 && couponData.map(item => <CouponItem showBtn={false} coupon={item} key={item.couponsId} scene={4} />)}
            </View>
