...
const packRequest = newRequestObj => {
  let response;
  return Taro.request(newRequestObj).then(resp => {
...
const request = ({ baseURL = MNP_REQUEST_DEFAULT_CONFIG.baseURL, url, data, method, auth = true, absolute = false, dealLoginSelf = false, dealErrorSelf = false }) => {
