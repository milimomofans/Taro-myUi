...
export default class CanvasDrawer extends Component {
  ...
  _downloadImageAndInfo = (image, index, pixelRatio) => {
    return new Promise((resolve, reject) => {
      downloadImageAndInfo(image, index, this.toRpx, pixelRatio).then(result => {
  ...
  onCreate = () => {
    ...
    return this.downloadResourceTransit().then(() => {
      this.create(config);
    }).catch(err => {
  ...
  create = config => {
    ...
    this.setState({
      ...
      this.initCanvas(config.width, height, config.debug).then(() => {
        ...
        const queue = this.drawArr.concat(texts.map(item => {
          item.type = 'text';
